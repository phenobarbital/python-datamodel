...
def test_datamodel():
    ...
    def test_model():
        ...
        results = rs_parsers.parse_datamodel(user)
...
def test_dates():
    ...
    def test_rst_date():
        for _ in range(10):
            dt = rs_parsers.to_date('2024-01-31 00:00:00')
            assert dt.year == 2024
            assert dt.month == 1
            assert dt.day == 31
            dt = rs_parsers.to_date('31-12-2022')
            assert dt.year == 2022
            assert dt.month == 12
            assert dt.day == 31
            dt = rs_parsers.to_date('12/31/2023')
            assert dt.year == 2023
            assert dt.month == 12
            assert dt.day == 31
...
def test_booleans():
    ...
    print('Test Boolean with Cython: ')
    ...
    def test_rstboolean():
        for _ in range(10):
            user = {
                "is_active": rs_parsers.to_boolean('true'),
                "is_admin": rs_parsers.to_boolean('false'),
                "is_staff": rs_parsers.to_boolean('yes'),
                "is_superuser": rs_parsers.to_boolean('No')
            }
            assert user["is_active"] is True

    print('Test Boolean with Rust: ')
    ...
    print('Test to_boolean with Cython: ')
    time = timeit.timeit(test_cython_boolean, number=1000)
    print(f"Execution time: {time:.6f} seconds")

    def test_rust_boolean():
        for val in inputs:
            rs_parsers.to_boolean(val)

    print('Test to_boolean with Rust: ')
