...
- **Codebase-first, then ask.** If ``wikitoolkit`` is available, use it as
  the fastest orientation source before grep/glob/read. Otherwise, rely on
  Grep, Glob, and Read for codebase search. The codebase itself is the
  primary evidence source. Never ask the user something the repo can answer.
...
REPO_ROOT=$(pwd)
```
You must be on `main` or the integration branch when starting. The
`base_branch` for the proposal defaults to `main` for all flows.

**Validation:** `base_branch` defaults to `main` for all flow types.
...
Before generating the plan, probe whether the knowledge graph is available:
```bash
wikitoolkit status 2>&1
```
If ``wikitoolkit`` is not installed or the output contains "Wiki not built",
set `wiki_available: false` in `state.json` and inform the planner so it
omits `wiki_*` query types and relies exclusively on Grep, Glob, Read, and
git_log queries. Otherwise set `wiki_available: true`.
...
1. **Budget check.** Compare `consumed` vs `budget`. If any limit will be
   ...
   **Wiki queries (`wiki_query`, `wiki_page`, `wiki_related`) are free —
   always execute them regardless of budget (only when `wiki_available: true`).**
2. **Execute** the query using the appropriate tool:
   - **If `wiki_available: true`:**
   ...
   - **If `wiki_available: false`** (or wikitoolkit not installed):
     - Use Grep with targeted patterns to locate relevant code.
     - Use Glob to discover file structure.
     - Use Read to examine discovered files.
     - These standard tools replace all wiki_* query types in the plan.
...
3. **Persist a digest** at `${STATE_DIR}/findings/F<NNN>-<slug>.md` using the
   ...
   findings (when available), cite the wiki page ID and score alongside
   file paths.
