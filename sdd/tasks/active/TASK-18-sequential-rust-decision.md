# TASK-18: Measure sequential Rust parity and promotion eligibility
...
**Depends-on**: TASK-17
**Parallel**: true
**Parallelism notes**: Own native harness and reports after native implementation; can coexist with serialization editing, but not simultaneous timing.
...
Implements M5 / M8 of the approved specification; contributes to AC8, AC11. Constructor/ORM hydration performance is the priority, with strict compatibility for repository schema examples and real `asyncdb.models`. Rust, parallel execution and serialization are measured experiments, not automatic backend migrations.
...
- Compare the candidate native executor against the frozen optimized Cython artifact using identical models, inputs and observable contracts.
- Include Python/native boundary, extraction/reconstruction, error construction and plan creation/reuse costs in appropriate warm/cold results.
- Exercise numeric/temporal/Unicode/regex boundaries and all ineligible/custom paths through the reference runner.
- Record an explicit promote-eligible or retain-Cython decision; packaging promotion itself is outside this task.

**NOT in scope**: Changing production Cython or Rust source, installing a default backend, or interpreting an inconclusive run as success.
...
| `tests/compatibility/test_native_executor.py` | MODIFY | Compatibility fixtures, test support or regression tests |
| `benchmarks/results/compatible-model-performance/rust-sequential.json` | CREATE | Reproducible evidence and decision report |
| `benchmarks/results/compatible-model-performance/rust-sequential.md` | CREATE | Reproducible evidence and decision report |
...
The harness/private interface is new output from the dependency; re-read its actual definition before extending it. If an incompatibility needs source changes, route the fix to the preceding executor task.
...
The verified contract above is the source for existing behavior. New interfaces from `TASK-17` must be documented with their actual definitions, never inferred from proposed filenames.

## Acceptance Criteria

- [ ] AC8 full-cost measurements and parity are persisted with raw samples and eligibility definition.
- [ ] Promotion eligibility requires >=10% additional improvement and all applicable <=5% regression/parity gates.
- [ ] Negative result is a valid completed experiment only with measurements and Cython retained; unfinished builds or missing evidence are not negative results.
- [ ] Required tests and evidence are recorded with actual commands/results; no new reference-relative regression is hidden.
- [ ] Only scoped files and task/index state change; AC8, AC11 evidence is linked in the completion note.

## Test Specification

- Run the native parity test file and benchmark all declared supported/ineligible cases with release artifacts.
- Verify no callback duplication and that fallback costs appear in the reported distribution.
- Validate decision against raw process ratios rather than direct scalar microbenchmarks.
...
7. Fill the completion note, then use `scripts/sdd/close_task.sh TASK-18 compatible-model-performance verified` only when all required criteria are met. Verify the active copy is gone and the per-spec index points to the completed file.
