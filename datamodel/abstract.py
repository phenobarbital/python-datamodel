...
def _dc_method_setattr_(self, name: str, value: Any) -> None:
    ...
    # FEAT-2/TASK-15 RETRACTED: this `if name not in self.__fields__:` guard
    # was removed as "provably redundant" and that reasoning was WRONG.
    # `object.__setattr__` above is not inert: it consults the type's data
    # descriptors, so a class-level `property` setter for `name` runs user
    # code -- which may legitimately append `name` to `self.__fields__` (a
    # self-registering property). Re-checking here is exactly what catches
    # that. Verified against 0.10.21: with the guard removed, such a class
    # raised `TypeError: Field 'dynamic' is not allowed` while the reference
    # accepted the assignment. Pinned by
    # tests/compatibility/test_structural_parity.py::
    # test_a_property_setter_may_register_its_own_field.
